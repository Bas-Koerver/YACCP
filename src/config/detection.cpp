...
namespace YACCP::Config {
        void parseDetectionConfig(const toml::table &tbl, DetectionConfig& config) {
            ...
            config.openCvDictionaryId = (*detectionTbl)["charuco_dictionary"].value_or(Defaults::charucoDictionary);
            config.detectionInterval = (*detectionTbl)["detection_interval"].value_or(Defaults::detectionInterval);
            config.cornerMin = (*detectionTbl)["minimum_corner_fraction"].value_or(Defaults::minCornerFraction);
