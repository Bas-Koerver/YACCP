...
namespace YACCP::Config {
    void parseRecordingConfig(const toml::table& tbl, RecordingConfig& config) {
        ...
        config.fps = (*recordingTbl)["fps"].value_or(Defaults::recordingFps);
        config.masterWorker = requireVariable<int>(*recordingTbl, "master_worker", "recording");

        // [[recording.workers]] array of tables variables.
        ...
        for (const toml::node& workerNode : *workerArray) {
            ...
            const WorkerTypes type{stringToWorkerType(requireVariable<std::string>(*workerTbl, "type", "[recording.workers]"))};
            typeCounts[type]++;
        }
        ...
        for (const toml::node& workerNode : *workerArray) {
            ...
            case WorkerTypes::prophesee: {
                Prophesee prophesee{};
                prophesee.accumulationTime = (*workerTbl)["accumulation_time"].value_or(Defaults::accumulationTime);
                prophesee.saveEventFile = (*workerTbl)["save_event_file"].value_or(false);
                prophesee.fallingEdgePolarity = requireVariable<int>(*workerTbl, "falling_edge_polarity", "[recording.workers]");
